...
score_low_gradient <- function(metric, values) {
  ...
  score <- switch(
    metric,
    "rich_coe" = ((values - 4.88) / 4.26) * 5 + 5,
    "rich_insect" = ((values - 22) / 11.12) * 5 + 5,
    "rich_genus" = ((values - 28.88) / 8.15) * 5 + 5,
    "pct_gatherer" = (((values - 56.13) / 22.24) * -1) * 5 + 5,
    "shannon_diversity_shredders" = ((values - 1.16) / 0.89) * 5 + 5,
  ...
  final_score <- ifelse(
    test = score > 10,
    yes = 10,
    no = ifelse(test = score < 0, yes = 0, no = score)
  )
